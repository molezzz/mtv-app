...
abstract class MovieRemoteDataSource {
  ...
  Future<List<VideoModel>> searchVideosFromSource(
...
class MovieRemoteDataSourceImpl implements MovieRemoteDataSource {
  ...
  Future<List<VideoModel>> searchVideos(String query) async {
    try {
      ...
      final videos =
          (response.data['results'] as List? ?? []).map((searchJson) {
        // 确保所有必需字段都有非空值
        final convertedJson = <String, dynamic>{
          'vod_id': (searchJson['id']?.toString() ?? '').isEmpty
              ? 'unknown'
              : searchJson['id']?.toString() ?? 'unknown',
          'vod_name': (searchJson['title']?.toString() ?? '').isEmpty
              ? 'Unknown Title'
              : searchJson['title']?.toString() ?? 'Unknown Title',
          ...
          'source_name':
              searchJson['source_name']?.toString(), // 添加source_name映射
        };
      ...
      if (filteredVideos.isEmpty && videos.isNotEmpty) {
        ...
        final videosWithSimilarity = videos.map((video) {
          final similarity =
              TitleMatcher.calculateSimilarity(query, video.title ?? '');
  ...
  @override
  Future<List<VideoModel>> searchVideosFromSource(
      String query, String resourceId) async {
    try {
      final response = await _apiClient.dio.get(
        '/api/search/one',
        queryParameters: {
      ...
      print(
          '特定资源($resourceId)原始结果: ${videos.length}, 过滤后: ${filteredVideos.length}');
    ...
    try {
      ...
      if (response.data is Map<String, dynamic>) {
        ...
        if (data.containsKey('list')) {
          ...
          final movies = movieList.map((movieJson) {
            // 创建新的Map以避免修改原始数据
            final Map<String, dynamic> newMovieJson =
            ...
            if (newMovieJson['poster'] != null) {
              ...
              if (!originalUrl.contains('/api/image-proxy')) {
                final proxiedUrl =
                    _imageProxyService.getProxiedImageUrl(originalUrl);
        ...
        if (data.containsKey('subjects')) {
          ...
          final movies = movieList.map((movieJson) {
            ...
            if (movieJson['poster'] != null) {
              movieJson['poster'] =
                  _imageProxyService.getProxiedImageUrl(movieJson['poster']);
            }
  ...
  Future<List<DoubanMovieModel>> getDoubanCategories({
    ...
    try {
      ...
      if (response.data is Map<String, dynamic>) {
        ...
        // 如果是新的API格式，使用list字段
        if (data.containsKey('list')) {
          final movies = (data['list'] as List? ?? []).map((movieJson) {
            // 处理图片代理
            if (movieJson['poster'] != null) {
              movieJson['poster'] =
                  _imageProxyService.getProxiedImageUrl(movieJson['poster']);
        ...
        // 如果是豆瓣原始格式，使用subjects字段
        if (data.containsKey('subjects')) {
          final movies = (data['subjects'] as List? ?? []).map((movieJson) {
            // 处理图片代理
            if (movieJson['poster'] != null) {
              movieJson['poster'] =
                  _imageProxyService.getProxiedImageUrl(movieJson['poster']);
      ...
      // 如果直接是数组格式
      if (response.data is List) {
        final movies = (response.data as List).map((movieJson) {
          // 处理图片代理
          if (movieJson['poster'] != null) {
            movieJson['poster'] =
                _imageProxyService.getProxiedImageUrl(movieJson['poster']);
